        ...
        for (const nft of nftsForSale) {
            ...
            nftCard.innerHTML = `
                ...
                <div class="nft-info">
                    ...
                    <button onclick="goToBuyPage(${nft.tokenId}, '${nft.price}')" class="primary-button">Buy</button>
                </div>
            `;
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const spinner = document.getElementById("loading");
    const statusMessage = document.getElementById("buy-status");
    ...
    try {
        ...
        let imageUrl = parsedMetadata.image || "placeholder.png";
        if (imageUrl.startsWith("ipfs://")) {
            imageUrl = imageUrl.replace("ipfs://", "https://gateway.pinata.cloud/ipfs/");
        }
        nftImage.src = imageUrl;
        nftImage.onerror = () => (nftImage.src = "placeholder.png");
    } catch (error) {
        console.error("Error loading NFT metadata:", error);
        showToast("Error loading NFT details. Please try again.", "error");
    }
    ...
    buyButton.addEventListener("click", async () => {
        try {
            ...
            //statusMessage.textContent = "Purchase complete! Redirecting to gallery...";
            //statusMessage.style.display = "block";
